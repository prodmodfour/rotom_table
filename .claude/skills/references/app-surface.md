...
**Accuracy gate (PTU p.214, decree-042):** Poke Ball throws are AC 6 Status Attack Rolls with the full accuracy system. `rollAccuracyCheck(params?: CaptureAccuracyParams)` accepts thrower accuracy stage, target Speed Evasion, flanking penalty, and rough terrain penalty. Uses `calculateAccuracyThreshold(6, accuracyStage, speedEvasion)` from `damageCalculation.ts` — the same utility used by `useMoveCalculation.ts`. Returns `{ roll, isNat1, isNat20, hits, threshold }`. Natural 1 always misses, natural 20 always hits, otherwise roll >= threshold. On miss, the Standard Action is consumed but no capture attempt occurs. `CombatantCaptureSection.vue` computes accuracy params from encounter combatant data (trainer accuracy CS, target speedEvasion). Both `handleApproveCapture` (GM-side) and `attempt.post.ts` (server-side) enforce this gate. The ack to the player includes `accuracyHit: boolean` so the UI can distinguish misses from capture failures.
