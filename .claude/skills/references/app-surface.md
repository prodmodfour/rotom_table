...
**Trainer level-up:** `utils/trainerAdvancement.ts` (pure advancement logic — `computeTrainerLevelUp`, `computeTrainerAdvancement`, `summarizeTrainerAdvancement`, milestone definitions, lifestyle stat point calculation), `composables/useTrainerLevelUp.ts` (reactive workflow state — stat allocation, maxHp preview, update payload builder), `components/levelup/LevelUpModal.vue` (wizard modal — stats -> summary steps, P1 will add edges/features/classes), `components/levelup/LevelUpStatSection.vue` (stat point allocation with evasion preview), `components/levelup/LevelUpSkillSection.vue` (skill rank allocation — disabled per decree-037, retained for P1 Edge integration), `components/levelup/LevelUpSummary.vue` (review step with P1 indicators and warnings). Integration: level watcher in `CharacterModal.vue` and `gm/characters/[id].vue` intercepts level increase, opens LevelUpModal, applies results to editData with `isApplyingLevelUp` guard to prevent double-trigger.
