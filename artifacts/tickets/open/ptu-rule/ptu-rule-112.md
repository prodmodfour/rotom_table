...
### Fix Cycle (slave/2-developer-ptu-rule-112-fix-20260227)

Addresses code-review-201 CHANGES_REQUIRED findings.

**Commits:**
- `fc948b7` test: add unit tests for getCombatantNaturewalks and naturewalkBypassesTerrain (24 tests)
- `84c04ac` test: add Naturewalk rough terrain bypass tests to useMoveCalculation (5 tests)
- `8b2612f` fix: handle earth terrain base cost in Naturewalk slow bypass (bug found during testing)
- `0ca2657` test: add Naturewalk slow terrain bypass tests to useGridMovement (8 tests)
- `0ac087f` docs: update app-surface.md with Naturewalk utility entries
- `bd05f40` docs: add format assumption comment to parseNaturewalksFromOtherCaps

**Files changed:**
- `app/tests/unit/utils/combatantCapabilities.test.ts` — NEW: 24 tests for getCombatantNaturewalks + naturewalkBypassesTerrain
- `app/tests/unit/composables/useMoveCalculation.test.ts` — +5 Naturewalk rough terrain bypass tests
- `app/tests/unit/composables/useGridMovement.test.ts` — +8 Naturewalk slow terrain bypass tests
- `app/composables/useGridMovement.ts` — Fix earth terrain Naturewalk bypass returning Infinity instead of 1
- `.claude/skills/references/app-surface.md` — Add constants/naturewalk.ts and new Naturewalk functions
- `app/utils/combatantCapabilities.ts` — Add documenting comment to parseNaturewalksFromOtherCaps

**Findings resolved:**
- HIGH-1: 37 unit tests added covering all Naturewalk code paths
- MED-1: app-surface.md updated with constants/naturewalk.ts and new functions
- MED-2: Documenting comment added to parseNaturewalksFromOtherCaps regex

**Bug discovered during testing:**
- `getTerrainCostForCombatant` Naturewalk slow bypass path returned `TERRAIN_COSTS['earth']` (Infinity) for earth terrain instead of 1 for burrowing combatants. Fixed with explicit earth check in bypass path.
