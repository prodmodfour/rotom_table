...
| 2026-03-04 | P2 fix cycle | 6 commits addressing code-review-321 (1C+3H+3M) + rules-review-294 (2H+3M). C1: split living-weapon.service.ts into abilities + movement sub-services (3319923a). H1: persist wieldMovementUsed on wielder combatant for mid-round reconstruction (eb9d7385). H2: reset wieldMovementUsed at round start in resetCombatantsForNewRound (d14f4492). H3: add No Guard client-side check per decree-046 +3/-3 flat accuracy in useMoveCalculation.ts + fix server-side to apply both attacker/target bonuses (f6acb5b5). M1: immutable update for wieldRel.movementUsedThisRound (a51fe8ac). M2: explicit JSDoc on applySoulstealerHealing mutation pattern (in C1 split). M3: Soulstealer scene frequency enforcement via featureUsage tracking (in C1 split). rules-MEDIUM-002: thread movement modifiers through shared pool in useGridMovement + living-weapon-movement.service (2d5e1260). Files: `app/server/services/living-weapon.service.ts`, `app/server/services/living-weapon-abilities.service.ts`, `app/server/services/living-weapon-movement.service.ts`, `app/server/services/living-weapon-state.ts`, `app/server/utils/turn-helpers.ts`, `app/server/api/encounters/[id]/position.post.ts`, `app/server/api/encounters/[id]/calculate-damage.post.ts`, `app/types/encounter.ts`, `app/composables/useMoveCalculation.ts`, `app/composables/useEncounterActions.ts`, `app/composables/useGridMovement.ts` |
