---
last_updated: 2026-03-01T14:25:00
updated_by: slave-collector (plan-20260301-135300)
---
...
| feature-006 | P1 | **P2-implemented** | Pokemon Evolution System — P0 APPROVED. P1 APPROVED. P2 implemented by slave-4 (plan-20260301-135300): 9 commits — Everstone/Eviolite prevention, item consumption for stone/held-item evolutions, post-evolution undo (snapshot + endpoint + composable), evolution history logging in Pokemon notes, gender-specific triggers, P2 features wired into EvolutionConfirmModal + XpDistributionResults, requiredGender/requiredMove exposed in evolution check response. Needs review | multi-phase |
...
| feature-011 | P1 | **P2-implemented** | Pokemon Switching Workflow — P0 APPROVED. P1 APPROVED. P2 implemented by slave-5 (plan-20260301-135300): 8 commits — immediate-act logic for released Pokemon (Section K), findAdjacentPosition + checkRecallReleasePair utilities, standalone recall endpoint (Section L), standalone release endpoint (Section L), recall/release store actions + composable methods, pair detection in recall endpoint (Section N), enhanced player switch request with recall/release details (Section M). Needs review | multi-phase |
...
| feature-009 | P1 | **P0-needs-fix-cycle** | Trainer XP & Advancement Tracking — P0 implemented by slave-2 (plan-20260301-130000). Reviewed: code-review-246 **CHANGES_REQUIRED** (H1: stale data in CharacterModal after XP award, H2: app-surface.md, M1: xpToNextLevel negative at max level, M2: duplicate award logic, M3: console.log). rules-review-222 **APPROVED** (10 mechanics verified, 0 rule violations, M1: capturedSpecies naming advisory for P1). Needs fix cycle | multi-phase |
| feature-013 | P1 | **P0-APPROVED** | Multi-Tile Token System — P0 fix cycle APPROVED. code-review-245 **APPROVED** (MED-1: useIsometricRendering.ts 820 lines → refactoring-111). rules-review-221 **APPROVED** (7 mechanics verified, 0 issues, all code-review-242 fixes confirmed correct, decree-003 compliant). P1 pending | multi-phase |
...
| feature-016 | P2 | **P0-needs-fix-cycle** | Priority / Interrupt / Attack of Opportunity System — P0 implemented. Reviewed: code-review-247 **CHANGES_REQUIRED** (CRIT-001: fainted reactor can execute AoO, H1: triggerType input not validated, H2: AOO_STRUGGLE_ATTACK_DAMAGE_BASE incorrect value 10→11, H3: app-surface.md, M1-M4: client preview eligibility, auto-decline on faint, stale record, pending actions accumulation). rules-review-223 **APPROVED** (14 mechanics verified, 0 rule violations, M1: AC 4 hardcoded, M2-M3: faint auto-decline advisory). Needs fix cycle | multi-phase |
...
**Current task:** Session 74 collection complete. 5 slaves merged (23 commits). 3 reviewer slaves (feature-013-p0-rereview APPROVED, feature-009-p0-review CHANGES_REQUIRED, feature-016-p0-review CHANGES_REQUIRED). 2 dev slaves (feature-006-p2 9 commits, feature-011-p2 8 commits). Smoke test PASSED. 1 follow-up ticket filed (refactoring-111).
