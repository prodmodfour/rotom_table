---
last_updated: 2026-03-02T10:42:00
updated_by: slave-collector (plan-20260302-084714)
---
...
| bug-042 | P2 | **BLOCKED** | release-hold.post.ts creates duplicate turnOrder entry in Full Contact battles after hold-release. Fixed by slave-1 (plan-20260302-081436). Reviewed by slave-4 (plan-20260302-084714): code-review-268 **BLOCKED** (CRIT-001: dedup logic searches wrong direction for hold-release — original entry is BEFORE insertion point, indexOf misses it; HIGH-001: tests not actually executed, would fail on CRIT-001). rules-review-244 **APPROVED** (incorrectly — analysis missed directional issue). **Fix does NOT resolve bug-042.** Needs complete rewrite of dedup logic |
...
| feature-008 | P1 | **P1-APPROVED** | Trainer Level-Up Milestone Workflow — P0 APPROVED. P1 fix cycle APPROVED: code-review-244 **APPROVED** (0 issues) + rules-review-220 **APPROVED** (12 mechanics verified, all 4 decrees compliant, 0 issues). All code-review-239 + rules-review-215 issues resolved. **Feature complete** (design has P0+P1 only, no P2 tier) | multi-phase |
...
| feature-014 | P1 | **P1-implemented** | VTT Flanking Detection — P0 APPROVED. P1 implemented by slave-1 (plan-20260302-084714): 5 commits — countAdjacentAttackerCells for multi-tile attacker flanking, findIndependentSet + checkFlankingMultiTile for P1, switched useFlankingDetection to checkFlankingMultiTile, comprehensive unit tests, ticket/design updates. Needs review | multi-phase |
| feature-016 | P2 | **P2-implemented** | Priority / Interrupt / Attack of Opportunity System — P0 APPROVED. P1 APPROVED. P2 implemented by slave-3 (plan-20260302-084714): 9 commits — extended Maneuver interface with shift/free types, Disengage maneuver, lineOfAttack utility (Bresenham), Intercept Melee/Ranged detection+resolution in out-of-turn service, intercept-melee+intercept-ranged+disengage endpoints, store actions, InterceptPrompt.vue component, ticket/design updates. Also resolves ptu-rule-095 (Disengage). Needs review | multi-phase |

| feature-017 | P2 | **P0-APPROVED** | Poke Ball Type System — P0 implemented by slave-2 (plan-20260302-081436). Reviewed by slave-5 (plan-20260302-084714): code-review-269 **APPROVED** (H1: missing ball modifier test coverage, M1: ineffective `as const`, M2: app-surface.md — address before P1) + rules-review-245 **APPROVED** (all 25 ball types verified correct vs PTU 1.05, all costs correct, conditional descriptions correct, decree-013/014/015 compliant). P0 approved, issues to address before P1 | multi-phase |
...
| feature-020 | P2 | **P0-fix-cycle-done** | Healing Item System — P0 implemented by slave-2 (plan-20260301-223500). code-review-267 CHANGES_REQUIRED + rules-review-243 APPROVED. Fix cycle by slave-2 (plan-20260302-084714): 6 commits — removed double validation (H1), used useCombatantDisplay + effective maxHp (H2/M1), deleted dead getApplicableItems stub (H3), simplified validation error (M2), updated app-surface.md (M3), replaced hardcoded 3px gap with $spacing-xs (M4). All 7 issues addressed. Needs re-review | multi-phase |
| feature-023 | P2 | **P0-CHANGES_REQUIRED** | Player Capture & Healing Interfaces — P0 implemented by slave-3 (plan-20260302-081436). Reviewed by slave-6 (plan-20260302-084714): code-review-270 **CHANGES_REQUIRED** (CR-1: gm/index.vue 812 lines over 800 cap; HI-1: unused encounterId prop; HI-2: no undo snapshots for capture/breather/healing; HI-3: alert() blocking errors; ME-1: app-surface.md; ME-2: captureRate % misleading; ME-3: empty deny reason; ME-4: import path inconsistency) + rules-review-246 **CHANGES_REQUIRED** (CRIT-001: 'Poke Ball' vs 'Basic Ball' string mismatch blocks all captures; HIGH-001: ballType not passed to attemptCapture; MED-001: entityId vs combatantId for healing items; MED-002: null check for failed capture). Needs fix cycle | multi-phase |
...
**Current task:** Session 83 collection complete. 6 slaves merged (26 commits total). 3 dev slaves (feature-014-p1 5 commits, feature-020-fix 6 commits, feature-016-p2 9 commits). 3 reviewer slaves (bug-042 BLOCKED, feature-017-p0 APPROVED, feature-023-p0 CHANGES_REQUIRED). Smoke test PASSED. No conflicts.
