...
import { useIsometricMovementPreview } from '~/composables/useIsometricMovementPreview'
...
export function useIsometricRendering(options: UseIsometricRenderingOptions) {
  ...
  // P1: Movement preview composable for cell highlights, range, and arrow
  const movementPreviewComposable = useIsometricMovementPreview({
    tokens: options.tokens,
    getTokenElevation: options.getTokenElevation,
    getTerrainElevation: options.getTerrainElevation,
  })
  ...
  const render = () => {
    ...
    if (options.movementRangeCells?.value && options.movementRangeCells.value.length > 0) {
      movementPreviewComposable.drawMovementRange(ctx, options.movementRangeCells.value, angle, gridW, gridH, cellSize)
    }
    ...
    if (options.hoveredCell?.value && isInBounds(options.hoveredCell.value, gridW, gridH)) {
      movementPreviewComposable.drawCellHighlight(ctx, options.hoveredCell.value, 0, angle, gridW, gridH, cellSize, HOVER_FILL, HOVER_STROKE)
    }
    ...
    if (options.movementPreview?.value) {
      movementPreviewComposable.drawMovementArrow(ctx, options.movementPreview.value, angle, gridW, gridH, cellSize)
    }
