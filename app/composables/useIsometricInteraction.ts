  ...
  const handleTouchEnd = (event: TouchEvent) => {
    ...
    if (event.touches.length === 0 && !isTouchPanning.value && touchStartPos.value) {
      ...
      if (changedTouch) {
        ...
        // Use screen-based hit-test for isometric accuracy
        const clickedToken = getTokenAtScreenPosition(changedTouch.clientX, changedTouch.clientY)
