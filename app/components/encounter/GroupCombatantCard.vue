<template>
  <div
    class="group-combatant"
    :class="{
      'group-combatant--current': isCurrentTurn,
      'group-combatant--fainted': isFainted
    }"
  >
    <!-- Sprite -->
    ...
    <!-- Info -->
    <div class="group-combatant__info">
      <h3 class="group-combatant__name">{{ displayName }}</h3>
      ...
      <div class="group-combatant__health">
    ...
    <!-- Turn Indicator -->
    <div v-if="isCurrentTurn" class="group-combatant__turn-indicator">
...
.group-combatant {
  ...
  &--current {
    ...
    // 4K optimization - more prominent effect
    @media (min-width: 3000px) {
      transform: scale(1.03);
      box-shadow: $shadow-glow-scarlet, 0 0 60px rgba($color-accent-scarlet, 0.3);
    }
  }
  ...
  &__sprite {
    ...
    // 4K optimization - larger sprites
    @media (min-width: 3000px) {
      width: 240px;
      height: 240px;
      border-width: 3px;
