...
The [[move-energy-system]] tracks both `usedToday` and `usedThisScene` counters. When a daily move is used, both counters increment. The scene counter resets via [[scene-activation-resets-move-counters]] but the daily counter persists until a rest.

## See also

- [[move-energy-system]] — enforces both daily and per-scene limits
