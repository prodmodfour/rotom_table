...
- [[grid-isometric-interaction-duplication]] — 85% code duplication between two composables that differ only in coordinate transform — the defining case for composition via injected strategy
- [[entity-data-model-rigidity]] — the Pokemon/HumanCharacter hierarchy is rigid because identity is modeled through type inheritance rather than composed capabilities
- [[trait-composed-domain-model]] — a proposal that applies this principle to the Combatant via narrow composed trait interfaces
