...
Total evasion has a minimum of 0. The `calculateEvasion` utility in `utils/damageCalculation.ts` implements this.
...
The threshold has a minimum of 1. The `calculateAccuracyThreshold` utility computes this.

The `useMoveCalculation` composable auto-selects the best evasion type (physical, special, or speed) for each target based on which gives the highest evasion.
...
- [[flanking-detection-utility]] — flanking applies an evasion penalty
