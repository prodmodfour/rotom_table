...
1. **`generatePokemonData(input)`** — Pure function. Looks up SpeciesData, rolls gender, assigns initial traits and moves, distributes stat points (5 × level), calculates HP. Returns `GeneratedPokemonData` (no DB write).
...
- [[species-data-model]] — species lookup for stats, traits
