...
| POST | `/api/capture/attempt` | Execute capture using [[capture-roll-mechanics|1d100 roll]]. Validates [[capture-accuracy-gate|accuracy gate]] when `accuracyRoll` provided. On success: sets ownerId, origin=`captured`, [[pokemon-loyalty|loyalty=2]]. Applies post-capture effects (Heal Ball, Friend Ball, Luxury Ball). Checks [[trainer-owned-species-tracking|new species]] for +1 trainer XP. Broadcasts `capture_attempt` WebSocket event |
