...
4. Broadcast `scene_activated` via [[scene-websocket-events|WebSocket]]
5. [[websocket-store-sync|BroadcastChannel]] notification for cross-tab sync

## Deactivation

1. Set scene `isActive = false`
2. Clear GroupViewState (`activeSceneId = null`)
3. Broadcast `scene_deactivated` via [[scene-websocket-events|WebSocket]]
