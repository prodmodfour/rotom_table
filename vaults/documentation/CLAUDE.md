...
~740 atomic notes describing how the PTR system will be implemented as the rotom_table app. Notes link to each other with `[[wikilinks]]`. Obsidian resolves links by filename regardless of folder.
...
## Domain prefixes (this directory, ~152 files)
...
- `vtt-*` / `grid-*` / `isometric-*` / `elevation-*` / `fog-*` / `depth-*` / `path-*` / `pathfinding-*` / `measurement-*` / `movement-*` / `multi-cell-*` / `custom-token-*` / `size-*` / `one-distance-*` / `three-coordinate-*` (~19) — VTT rendering, grid mode, interaction, projection, camera, measurement, elevation, fog of war, pathfinding, movement atomicity, token footprint
...
- Engine design (~20) — effect engine, entity model, game state interface, state delta model, field state interfaces, and related notes
