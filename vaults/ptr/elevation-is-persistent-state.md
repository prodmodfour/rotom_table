...
Vertical positioning — token elevation and terrain height — is real game state that must persist across turns, sessions, and views. PTU has vertical movement (flying via Sky [[trait-definition|trait]], levitating) that persists across turns. If elevation is ephemeral, the game state becomes inconsistent.
